...
import (
	...
	"go.datalift.io/admiral/server/mock/service/authnmock"
)
...
`, cfg)
	...
	assert.NotZero(t, claims.RegisteredClaims.IssuedAt)
	assert.Equal(t, claims.RegisteredClaims.ExpiresAt, time.Unix(claims.RegisteredClaims.IssuedAt, 0).Add(5*time.Hour).Unix())
	assert.Equal(t, claims.RegisteredClaims.Subject, "service:some subject")
	...
	assert.NotZero(t, claims.RegisteredClaims.IssuedAt)
	assert.Equal(t, claims.RegisteredClaims.ExpiresAt, int64(0))
	assert.Equal(t, claims.RegisteredClaims.Subject, "service:some subject")
...
`, cfg)
	...
	testcases := []struct {
		...
		{
			claims: jwt.RegisteredClaims{Subject: email},
		},
		{
			claims:                  jwt.RegisteredClaims{Subject: email},
		...
		{
			claims:      jwt.RegisteredClaims{Subject: email, ExpiresAt: time.Now().Add(-1 * time.Hour).Unix()},
