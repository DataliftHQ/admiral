const GRPC_CODE_MAPPINGS: Record<number, { code: number; text: string }> = {
  0: { code: 200, text: 'OK' },
...
const grpcCodeToText = (code: number): string => {
  const mapping = GRPC_CODE_MAPPINGS[code]
  return mapping?.text !== undefined ? mapping.text : 'Unknown Code'
}

const grpcCodeToHttpCode = (code: number): number => {
  return GRPC_CODE_MAPPINGS[code]?.code !== undefined ? GRPC_CODE_MAPPINGS[code]?.code : 500
}
