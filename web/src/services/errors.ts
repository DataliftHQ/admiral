...
import { type ErrorDetails as ErrorDetailsApi } from '../api/common/v1/error_pb'
import { type HttpStatus } from './client.ts'
...
export interface AdmiralErrorDetails extends ErrorDetailsApi, Details {}
...
export interface AdmiralError extends Error {
...
/**
 * Construct a AdmiralError from an AxiosError.
 *
 * @param clientError A client error object.
 */
const grpcResponseToError = (clientError: AxiosError): AdmiralError => {
  const response = clientError?.response as AxiosResponse

  const error: AdmiralError = {
    code: response.data.code,
    message: response.data.message,
    status: {
      code: response.status,
      text: grpcCodeToText(response.data.code),
  ...
  if (response.data.details !== undefined && response.data.details.length > 0) {
    ...
    error.details = response.data.details.map((detail: any) => {
...
const isAdmiralErrorDetails = (details: ErrorDetails): details is ErrorDetails => {
  return details._type === 'type.googleapis.com/admiral.common.v1.ErrorDetails'
}

function isAdmiralError(error: any): error is AdmiralError {
  return (error as AdmiralError).status !== undefined
}

export { grpcResponseToError, isAdmiralErrorDetails, isAdmiralError, isHelpDetails }
